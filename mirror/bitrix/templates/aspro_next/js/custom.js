...
function clickCalcBtn(e){
    ...
    if (!$('.calc-popup-shadow').length) {
        ...
        html += '</div>';
        html += '<style>';
        html += '.calc-popup-shadow{display:none;position:fixed;top:0;left:0;width:100%;height:100%;z-index:10000;background:rgba(0,0,0,.4);}';
        html += '.calc-popup-wrap{display:none;position:fixed;top:0;left:0;width:100%;height:100%;z-index:10001;display:flex;justify-content:center;align-items:center;}';
        html += '.calc-popup-content{background:#fff;padding: 20px 20px 0 20px; margin: 20px; max-width: 500px;position:relative;z-index:10002}';
        html += '.calc-popup-content p{}';
        html += '.calc-popup-content span{border-radius: 50%;border: 1px solid #0088cc;display: inline-block;width: 26px;text-align: center;}';
        html += '.calc-popup-content input{width: 71px;}';
        html += '.calc-popup-content strong{}';
        html += '.calc-popup-content a{display: block;}';
        html += '</style>';
        $('body').append(html);
            ...
            resize = function(){
                if (!resizeActive) {
                    ...
                    if (bodyWidth >= 992 && bodyWidth <= 1710) {
                        var h = 470 - ((1710 - bodyWidth) * 0.27855),
