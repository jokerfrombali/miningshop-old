var ItemObj={};$(document).on('click',".item-stock .store_view",function(){scroll_block($('.stores_wrapper'));});function setNewHeader(){var ratingHtml=priceHtml=skuHtml=imgHtml=buttonHtml='';if(arNextOptions['THEME']['SHOW_HEADER_GOODS']!='Y')
return;if($('.item_main_info .rating').length)
...
if($('.offers_img.wof').length){if($('.item_slider .slides .popup_link').length){imgHtml=$('.item_slider .slides .popup_link').html().replace('id','data-id');}else{imgHtml=$('.item_slider .slides > .offers_img a:first').html().replace('id','data-id');}}else if($('.item_slider .slides > ul > li:first link').length){imgHtml='<img src="'+$('.item_slider .slides > ul > li:first link').attr('href')+'" />';}else{imgHtml=$('.item_slider .slides > ul > li:first a').html();}
if($('.prices_tab').length){buttonHtml='<span class="more btn btn-default btn-lg">'+BX.message('MORE_INFO_SKU')+'</span>';}else if($('.offers_img.wof').length){if($('.catalog_detail .buy_block .offer_buy_block .in-cart').is(':visible')){buttonHtml='<span class="buy btn btn-default btn-lg type_block in-cart">'+$('.catalog_detail .buy_block .offer_buy_block .in-cart').html()+'</span>';}else{buttonHtml='<span class="buy btn btn-default btn-lg type_block">'+BX.message('MORE_BUY_SKU')+'</span>';}}else if($('.middle_info .buy_block .button_block').length){buttonHtml=$('.middle_info .buy_block .button_block').html();}
if($('.sku_props .bx_catalog_item_scu > div').length)
{var skuHtmlTmp='';$('.catalog_detail .sku_props .bx_catalog_item_scu > div').each(function(){var _this=$(this),li_block=_this.find('li.active'),select_block=_this.find('select');if(li_block.length)
...
$('#headerfixed .logo-row').html('<div class="inner-table-block product_block table">'+
...
'</div>'+
'</div>');}
$(document).ready(function(){if($('.catalog_detail .stock_wrapper').length>1){$('.catalog_detail .stock_wrapper').first().html($('.stock_wrapper').last().html()).show();}
if(arNextOptions['THEME']['SHOW_HEADER_GOODS']=='Y')
$('#headerfixed .logo-row').addClass('wproduct');if(BX.message('TYPE_SKU')!='TYPE_1'||BX.message('HAS_SKU_PROPS')!='Y')
setNewHeader();$('<div class="product-item-detail-tabs-container-fixed">'+
...
'</ul>'+
'</div>'+
'</div>').insertAfter($('#headerfixed'));$('.product-item-detail-tabs-list a').addClass('dark_link');$('.tab_slider_wrapp .tabs > li').first().addClass('cur');$('.tab_slider_wrapp .slider_navigation > li').first().addClass('cur');$('.tab_slider_wrapp .tabs_content > li').first().addClass('cur');$('.logo-row .module_products_list .but-cell .more').click(function(){var offset=0;if($('.middle_info .sku_props').length)
offset=$('.middle_info').offset().top;else
...
{var content_offset=$('.tabs_section.type_more').offset(),tab_height=$('.product-item-detail-tabs-container-fixed').actual('outerHeight'),hfixed_height=$('#headerfixed').actual('outerHeight');$('html, body').animate({scrollTop:content_offset.top-hfixed_height-tab_height},400);}
...
{$(".shadow.common").show();$("#reviews_content").hide();}})
...
this.set_quantity=$(this.obProduct).find('input[name=quantity]').val();this.checkPriceRange(this.set_quantity);$(this.obProduct).find('.not_matrix').hide();$(this.obProduct).find('.with_matrix .price_value_block').html(getCurrentPrice(this.currentPrices[this.currentPriceSelected].PRICE,this.currentPrices[this.currentPriceSelected].CURRENCY,this.currentPrices[this.currentPriceSelected].PRINT_PRICE));$('.logo-row .price-cell').find('.with_matrix .price_value_block').html(getCurrentPrice(this.currentPrices[this.currentPriceSelected].PRICE,this.currentPrices[this.currentPriceSelected].CURRENCY,this.currentPrices[this.currentPriceSelected].PRINT_PRICE));if($(this.obProduct).find('.with_matrix .discount'))
{$(this.obProduct).find('.with_matrix .discount').html(getCurrentPrice(this.currentPrices[this.currentPriceSelected].BASE_PRICE,this.currentPrices[this.currentPriceSelected].CURRENCY,this.currentPrices[this.currentPriceSelected].PRINT_BASE_PRICE));$('.logo-row .price-cell').find('.with_matrix .discount').html(getCurrentPrice(this.currentPrices[this.currentPriceSelected].BASE_PRICE,this.currentPrices[this.currentPriceSelected].CURRENCY,this.currentPrices[this.currentPriceSelected].PRINT_BASE_PRICE));}
...
this.config.showClosePopup=!!this.params.CONFIG.SHOW_CLOSE_POPUP;this.config.offerID=this.params.SKU_DETAIL_ID}
else
...
{var objUrl=parseUrlQuery(),sku_params=this.config.offerID,sku_id=0;if(sku_params&&sku_params in objUrl)
sku_id=objUrl[sku_params];if(sku_id)
...
if('src'in this.currentBigImg){if(this.currentBigImg.src){if(this.config.mainPictureMode==='POPUP'){$(this.obPict).parent().attr('href',this.currentBigImg.src).addClass('popup_link');}
$(this.obPict).parent().attr('title',img.TITLE);$(this.obPict).parent().attr('alt',img.ALT);$(this.obPict).attr('title',img.TITLE);$(this.obPict).attr('alt',img.ALT);}}
if(this.config.mainPictureMode=='POPUP'){if(!('ID'in img)){$(this.obPict).parent().removeClass('popup_link');}}}};window.JCCatalogElement.prototype.scaleImg=function(src,dest)
{var
...
InitFlexSlider();var interval=setInterval(function(){if($slider.find('.slides .item').length&&$slider.find('.slides .item').attr('style').indexOf('height')===-1){$(window).resize();}
...
this.currentImg.src+
'" width="'+this.currentImg.width+'" height="'+this.currentImg.height+'" name=""></div>';this.obPopupPict.setContent(pictContent);this.obPopupPict.show();return BX.PreventDefault(e);};window.JCCatalogElement.prototype.QuantityUp=function()
...
this.SetAdditionalGallery(index);this.SetSliderPict(this.offers[index],this.offers[index].SLIDER,this.config);this.SetSliderPictMobile(this.offers[index],this.offers[index].SLIDER,this.config);if(this.config.showSkuProps&&$('.top_props .props.props_list').length)
{$('.sku_prop').remove();if(this.offers[index].DISPLAY_PROPERTIES)
{for(var i in this.offers[index].DISPLAY_PROPERTIES)
{var class_block=((this.offers[index].DISPLAY_PROPERTIES[i].HINT&&this.offers[index].DISPLAY_PROPERTIES[i].SHOW_HINTS=="Y")?' whint1':''),hint_block=((this.offers[index].DISPLAY_PROPERTIES[i].HINT&&this.offers[index].DISPLAY_PROPERTIES[i].SHOW_HINTS=="Y")?'<div class="hint"><span class="icon"><i>?</i></span><div class="tooltip">'+this.offers[index].DISPLAY_PROPERTIES[i].HINT+'</div></div>':'');html='<div class="prop sku_prop"><div class="name"><div class="char_name"><span class="'+class_block+'">'+hint_block+'<span>'+this.offers[index].DISPLAY_PROPERTIES[i].NAME+'</span></span></div></div><div class="value"><div class="char_value"><span>'+this.offers[index].DISPLAY_PROPERTIES[i].VALUE+'</span></div></div></div>';$('.top_props .props.props_list .inner_props').append(html);}}}
...
{var container=$('.wrapp_thumbs'),slideHtml='';countPhoto=obj.SLIDER_COUNT;container.find('.thumbs').html('');slideHtml='<div class="inner_slider"><ul class="slides_block">';for(var i in slider){if(typeof(slider[i])=='object'){slideHtml+='<li data-slide_key="'+i+'" data-big="'+slider[i].BIG.src+'">'+
'<span class="cnt"><img border="0" src="'+slider[i].THUMB.src+'" alt="'+slider[i].ALT+'" title="'+slider[i].TITLE+'" /></span>'+
'</li>';}}
slideHtml+='</ul></div>';container.find('.thumbs').html(slideHtml);container.find('.popup_video').remove();if(countPhoto>1||obj.POPUP_VIDEO){if(slider.length>1)
$('.item_slider').addClass('has_more').removeClass('has_one');if(obj.POPUP_VIDEO)
container.append('<div class="popup_video '+(slider.length>5?'fromtop':'')+'"><a class="various video_link" href="'+obj.POPUP_VIDEO+'">'+BX.message('POPUP_VIDEO')+'</a></div>');if(countPhoto>1)
{if(container.find('.thumbs_navigation').length){container.find('.thumbs_navigation').remove();}
var containerMain=$('.item_main_info .item_slider.has_more .slides');if(containerMain.find('.flex-direction-nav.ext').length){containerMain.find('.flex-direction-nav.ext').remove();}
containerMain.append('<ul class="flex-direction-nav ext"><li class="flex-nav-prev"><span class="flex-prev">Previous</span></li><li class="flex-nav-next"><span class="flex-next">Next</span></li></ul>');container.find('.thumbs').append('<span class="thumbs_navigation bx-controls-direction"><span class="slide-prev"></span><span class="slide-next"></span></span>');container.find('.thumbs').removeData("bxSlider");window["obSliderSKU"]=container.find('.thumbs .slides_block').bxSlider({mode:'vertical',minSlides:5,maxSlides:5,slideMargin:10,pager:false,adaptiveHeight:false,responsive:false,touchEnabled:false,oneToOneTouch:false,moveSlides:(slider.length>5?1:0),nextSelector:'.bx-controls-direction .slide-next',prevSelector:'.bx-controls-direction .slide-prev',preventDefaultSwipeY:true,onSliderLoad:function(index)
{if(slider.length>5)
{$(this).closest('.bx-viewport').addClass('long');$(this).closest('.bxSlider').find('.bx-controls-direction a').addClass('opacityv');}
$(this).find('li[data-slide_key="0"]').addClass('current');}})
container.show();container.find('.thumbs .slides_block li').first().addClass('current');}
else
{container.find('.thumbs').hide();}
var pict=$(this.obPict);container.find('.thumbs').delegate('li:not(.current)','click',function(e){var srcSmall='',srcBig='',width='',height=''
index=$(this).data('slide_key');if(typeof slider[index]!=='undefined')
...
$(this).addClass('current').siblings().removeClass('current').parents('.item_slider').find('.slides li').fadeOut(333);$(this).siblings('[data-slide_key='+index+']').addClass('current');$(this).parents('.item_slider').find('.slides li').eq(index).addClass('current').stop().fadeIn(333);if(srcSmall){BX.adjust(pict[0],{props:{src:srcSmall}});}
...
{var container=$('.item_slider.flex'),slideHtml='';countPhoto=obj.SLIDER_COUNT;slideHtml='<ul class="slides">';if(!slider.length){slideHtml+='<li data-value="">'+
'<span data-fancybox-group="item_slider" class="fancy" title="'+obj.NAME+'"><img border="0" src="'+obj.NO_PHOTO.SRC+'" alt="'+obj.NAME+'" title="'+obj.NAME+'" /></span>'+
'</li>';}else{for(var i in slider){if(typeof(slider[i])=='object'){slideHtml+='<li data-value="'+obj.ID+'_'+slider[i].ID+'">'+
...
if(obj.CONFIG.ACTION!=="NOTHING"){if(obj.CONFIG.ACTION=="ADD"&&obj.CAN_BUY&&obj.SHOW_ONE_CLICK_BUY!="N"){var ocb='<span class="transparent btn-lg type_block btn btn-default white one_click transition_bg" data-offers="Y" data-item="'+obj.ID+'" data-iblockID="'+obj.IBLOCK_ID+'" data-quantity="'+obj.CONFIG.MIN_QUANTITY_BUY+'" data-props="'+obj.OFFER_PROPS+'" onclick="oneClickBuy('+obj.ID+', '+obj.IBLOCK_ID+', this)">'+
...
buyBlock.fadeIn();buyBlock.find('.counter_wrapp .counter_block input').data('product','ob'+this.obProduct.id);this.setPriceAction('','Y');if(typeof obNextPredictions==='object'){obNextPredictions.showAll();}}
...
{if(parseFloat(quantity)>0){$(this.storeQuanity).find('.icon').removeClass('order stock_range_1 stock_range_2 stock_range_3').addClass('stock stock_range_'+indicators);}else{$(this.storeQuanity).find('.icon').removeClass('stock stock_range_1 stock_range_2 stock_range_3').addClass('order');}
