    ...
    var initCountdown = function initCountdown() {
        if ($('.view_sale_block').size()) {
            $('.view_sale_block').each(function () {
                ...
                $(this).find('.countdown').countdown({until: dateTo, format: 'dHMS', padZeroes: true, layout: '{d<}<span class="days item">{dnn}<div class="text">{dl}</div></span>{d>} <span class="hours item">{hnn}<div class="text">{hl}</div></span> <span class="minutes item">{mnn}<div class="text">{ml}</div></span> <span class="sec items">{snn}<div class="text">{sl}</div></span>'}, $.countdown.regionalOptions['ru']);
            })
        }
    }
}
    ...
    var oneClickBuy = function (elementID, iblockID, that) {
        ...
        if (window.matchMedia('(min-width:992px)').matches)
        {
            if (!$(that).hasClass('clicked'))
            {
                ...
                $('.' + name + '_frame').jqm({trigger: '.' + name + '_trigger', onHide: function (hash) {
                        onHidejqm(name, hash);
...
/*$(document).on('click', function(event) {
    if (!$(event.target).closest('.custom-catalog-button').length) {
        $('.new-custom-menu').toggle();
        $('.custom-catalog-img').toggle();
        $('.custom-catalog-img-hover').toggle();
    }
});
*/
$(document).on('click', '.custom-catalog-button', function () {
    let target = $('.new-custom-menu');
    $('.custom-catalog-img').toggle();
    $('.custom-catalog-img-hover').toggle();
    target.toggle();
    let buttonPos = document.querySelector('.custom-catalog-button').getBoundingClientRect();
    let top = buttonPos['top']+45;
    let left = buttonPos['left']-(document.querySelector('.new-custom-menu').getBoundingClientRect()['width'])/4;
    target.css({'position': 'absolute', 'z-index': 9999, 'left': left+'px', 'top': top+'px'});
});
    ...
    if ($('.logo_and_menu-row .search_wrap').length) {
        ...
        $('.logo_and_menu-row .maxwidth-theme > .row >div').each(function () {
            ...
            if ($('.logo_and_menu-row .search_wrap.wide_search').length)
                $('.logo_and_menu-row .search_wrap .search-block').outerWidth(width-130);
            else
...
$(document).ready(function () {
    ...
    if ($mobilefilter.length) {
        ...
        OpenMobileFilter = function () {
            if (!$mobilefilter.isOpen) {
                ...
                let filter_curr_data = $("#mobilefilter").html();
                if(!filter_curr_data) {
                    let filter_data = $("#smart-filter-main");
                    $("#mobilefilter").html(filter_data);
                    $("#mobilefilter").css({'overflow-y': 'scroll'});
                }
    ...
    $('.tabs>li').on('click', function () {
     ...
     $('.catalog_detail .tabs_section > .tabs> .nav-tabs > li').on('click', function () {
         var tab_id = $(this).children().attr("href")
                tab = $(tab_id ),
                clicked_tab_1 = $(this).index()+1;
                tab_panes = tab.find(".tab-content"),
                nav_tabs = tab.find(".nav-tabs");
                console.log(tab_id);
                console.log(clicked_tab_1);
                // $('.nav.nav-tabs li').each(function () {
                //if ($(this).index() == clicked_tab)
                //    $(this).addClass('active');
                //})
                ...
                nav_tabs.find("li:nth-child("+ clicked_tab_1 +")").addClass('active');
                tab_panes.find(".tab-pane").removeClass('active');
                tab_panes.find(".tab-pane:nth-child("+ clicked_tab_1 +")").addClass('active');
                console.log(tab_panes.find(".tab-pane:nth-child("+ clicked_tab_1 +")"));
...
if (!funcDefined('getActualBasket')) {
    getActualBasket = function (iblockID, type) {
        ...
        $.ajax({
            type: "GET",
            url: arNextOptions['SITE_DIR'] + "ajax/basket_data.php",
            success: function (data) {
                let info = JSON.parse(data);
                for (const key in info) {
                    $('#'+key+'Counter').html(info[key]);
                }
            }
        });
        $.ajax({
            type: "GET",
            url: arNextOptions['SITE_DIR'] + "ajax/basket_ajax.php",
            success: function (data) {
                $('#basketPopup').html(data);
                $("#basketPopup").fadeIn(200, function() {
                    setTimeout(() => {
                        $(this).fadeOut(400);
                    }, 3000);
                });
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    searchInput.addEventListener('keyup', function () {
        // Если пользователь вводит символ, сбрасываем предыдущий таймер
        ...
        searchTimer = setTimeout(function () {
            ...
            // Чтобы не делать пустых запросов, проверим длину 
            if (query.length > 1) {
                ...
                fetch('/ajax/search.php?q=' + encodeURIComponent(query))
                    ...
                    .then(function(html) {
                        // Подставляем верстку с результатами
                ...
                // Если ввели мало символов — очищаем результаты
                searchResults.innerHTML = '';
            }
        }, 300); // время «debounce»
    });
