...
$(document).ready(function (){
	...
	$('html, body').on('mousehover', function(e) {
		if(typeof e.target.className == 'string' && e.target.className.indexOf('adm') < 0)
		{
			e.stopPropagation();
			var hint = $(e.target).closest('.hint1');
			if(!$(e.target).closest('.hint1').length)
			{
				$('.hint1').removeClass("active").find(".tooltip").slideUp(100);
			}
			else
			{
...
window.JCCatalogElement.prototype.SetSliderPict = function(obj, slider, config)
{
	...
	container.find('.photos360').remove();
...
{
	BX.bind(document, 'onmouseover', BX.proxy(this.popupWindowClick, this));
};
